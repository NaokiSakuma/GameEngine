...
{
	//デバッグ表示がオフなら
	if (CollisionNode::CollisionNode::GetDebugVisible() == false)
	{
		// モデル描画
		m_Obj.Render();
	}
...
// reject : 押し出すベクトル
//--------------------------------------------------------------------------------------
bool LandShape::IntersectSphere(const Collision::Sphere& sphere, Vector3* reject)
{
	...
	//// 球をコピー
	//Collision::Sphere localsphere = sphere;
	...
	//	const Collision::Triangle& tri = m_pData->m_Triangles[i];
...
//--------------------------------------------------------------------------------------
bool LandShape::IntersectSegment(const Collision::Segment& segment, Vector3* inter)
{
	//if (m_pData == nullptr) return false;
	...
	//// コピー
	//Collision::	Segment localSegment = segment;
	...
	//// 線分の方向ベクトルを取得
	//Vector3 segmentNormal = localSegment.Snd - localSegment.Start;
